...
*/
	...
	public void jouer()
	{
		...
		System.out.println(tabJoueur[0].getPlateau().getTabTest().toString());

		System.out.println(tabJoueur[0].getNom()+ " tu as " + this.sommePoint(tabJoueur[0].getPlateau().getTabTest()) + "points");
		//System.out.println(tabJoueur[0].getNom()+ " tu as " + this.sommePoint(tabJoueur[0].getPlateau()) + "points");
		//System.out.println(tabJoueur[1].getNom()+ " tu as " + this.sommePoint(tabJoueur[1].getPlateau()) + "points");
	}
	...
	public int sommePoint(Plateau plateau)
	{
		...
		Biome biomeCourant = null;
		Biome biomePoint   = null;
		Biome[][] tabJeu = plateau.getTabJeu();
		//System.out.println(plateau.getTabTest.toString());
		int tmpi = 0;
		int tmpo = 0;
		boolean bOk = true;
		boolean vider = false;
		while(bOk)
		{
			ArrayList<Biome> sommet= new ArrayList<Biome>(); 
			for (int i = 0;i<5;i++)
				for (int j = 0;j<5;j++)
				{	
					if (tabJeu[i][j] != null && biomeCourant != null)
					{
						biomeCourant = tabJeu[i][j];
						tmpi = i;
						tmpo = j;
			...
			sommet.add(biomeCourant);
			for (int cptL = tmpi -1;cptL<tmpi+1;cptL+=2)
				try{
					if(tabJeu[cptL][tmpo] != null && tabJeu[cptL][tmpo].equals(biomeCourant) && tabJeu[cptL][tmpo].getNum() != biomeCourant.getNum())
					{
						sommet.add(tabJeu[cptL][tmpo]);
						vider = false;
						System.out.println("check");
					}
					else
			...
			for (int cptC = tmpo -1;cptC<tmpo+1;cptC+=2)
				try{
					if(tabJeu[tmpi][cptC] != null && tabJeu[tmpi][cptC].equals(biomeCourant) && tabJeu[tmpi][cptC].getNum() != biomeCourant.getNum())
					{
						sommet.add(tabJeu[tmpi][cptC]);
						vider = false;
					}
					else
					{
			...
			if (vider)
			{
				...
				{System.out.println("check");
					multiplicateur += sommet.remove(0).getPoint();	
					nbPiece++;
				}
			}
			else
